...
public struct AngularStop: View {
    ...
    private func calculateStopLocation(_ location: CGPoint) {
        var direction = calculateDirection(center, location)
        if start > end {
            if direction > start && direction < 1 {
                direction = (direction - start)/(end+1 - start)
                stop.location = CGFloat(direction)
            } else {
                direction = max(min(direction + 1 , end + 1), start)
                stop.location = abs(CGFloat((direction-start)/((end+1)-start) ))
            }
        } else {
            direction = max(min(direction, end), start)
            stop.location = CGFloat((direction-start)/(end-start) )
        }
    }
...
public struct AngularGradientPicker: View {
    ...
    private var currentStates: (start: Double, end: Double) {
        let start = startState + manager.gradient.startAngle
    ...
    private func startHandle(_ proxy: GeometryProxy) ->  some View {
        style.makeStartHandle(configuration: startConfiguration)
    ...
    private func endHandle(_ proxy: GeometryProxy) ->  some View {
        style.makeEndHandle(configuration: endConfiguration)
            .offset(endOffset(proxy))
            .position(currentCenter(proxy))
            .gesture(
                DragGesture(minimumDistance: 10, coordinateSpace: .named(space))
                    .onChanged({
                        let direction = calculateDirection(currentCenter(proxy), $0.location)
                        endState = direction - manager.gradient.endAngle
                    })
                    .onEnded({
                        manager.gradient.endAngle = calculateDirection(currentCenter(proxy), $0.location)
                        endState = 0
                    })
            )
    }
    ...
    private func stops(_ proxy: GeometryProxy) -> some View {
        ForEach(manager.gradient.stops.indices, id: \.self) { (i) in
            AngularStop(stop: $manager.gradient.stops[i],
                        selected: $manager.selected,
                        isHidden: manager.hideTools,
                        id: manager.gradient.stops[i].id,
                        start: currentStates.start,
                        end: endState + manager.gradient.endAngle,
                        center: currentCenter(proxy))
        }
        .position(currentCenter(proxy))
    }
